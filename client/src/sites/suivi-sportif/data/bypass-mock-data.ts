import type { Exercise, Food, Meal, NutritionGoal, Workout, WorkoutTemplate } from "../api/client";
import {
  bypassCreatedAt as createdAt,
  bypassUpdatedAt as updatedAt,
  bypassUserId,
} from "./bypass-data-constants";

export { bypassBodyMeasurements, bypassUserGoals } from "./bypass-goals-data";

export const bypassExercises: Exercise[] = [
  {
    id: "bypass-exercise-squat",
    name: "Squat barre",
    description: "Mouvement de base pour les jambes et le gainage.",
    difficulty: "INTERMEDIATE",
    exerciseType: "STRENGTH",
    createdAt,
    updatedAt,
  },
  {
    id: "bypass-exercise-bench-press",
    name: "Developpe couche",
    description: "Exercice de poussee pour les pectoraux, triceps et epaules.",
    difficulty: "INTERMEDIATE",
    exerciseType: "STRENGTH",
    createdAt,
    updatedAt,
  },
  {
    id: "bypass-exercise-row",
    name: "Rowing halteres",
    description: "Tirage horizontal pour renforcer le dos.",
    difficulty: "BEGINNER",
    exerciseType: "STRENGTH",
    createdAt,
    updatedAt,
  },
  {
    id: "bypass-exercise-plank",
    name: "Planche",
    description: "Gainage statique pour le tronc.",
    difficulty: "BEGINNER",
    exerciseType: "MOBILITY",
    createdAt,
    updatedAt,
  },
];

export const bypassWorkouts: Workout[] = [
  {
    id: "bypass-workout-full-body",
    userId: bypassUserId,
    name: "Full body reprise",
    date: "2026-05-01T17:30:00.000Z",
    status: "COMPLETED",
    duration: 55,
    notes: "Seance de reprise sans aller a l'echec.",
    createdAt,
    updatedAt,
    exercises: [
      {
        id: "bypass-workout-exercise-squat",
        exerciseId: "bypass-exercise-squat",
        order: 1,
        exercise: bypassExercises[0],
        sets: [
          {
            id: "bypass-set-squat-1",
            setNumber: 1,
            reps: 10,
            weight: 60,
            rpe: 7,
            rir: 3,
            rest: 90,
            createdAt,
          },
          {
            id: "bypass-set-squat-2",
            setNumber: 2,
            reps: 8,
            weight: 70,
            rpe: 8.5,
            rir: 1,
            rest: 120,
            createdAt,
          },
        ],
      },
      {
        id: "bypass-workout-exercise-bench-press",
        exerciseId: "bypass-exercise-bench-press",
        order: 2,
        exercise: bypassExercises[1],
        sets: [
          {
            id: "bypass-set-bench-1",
            setNumber: 1,
            reps: 10,
            weight: 50,
            rpe: 7.5,
            rir: 2,
            rest: 90,
            createdAt,
          },
          {
            id: "bypass-set-bench-2",
            setNumber: 2,
            reps: 8,
            weight: 55,
            rpe: 8.5,
            rir: 1,
            rest: 120,
            createdAt,
          },
        ],
      },
    ],
  },
  {
    id: "bypass-workout-core",
    userId: bypassUserId,
    name: "Dos et gainage",
    date: "2026-05-03T10:00:00.000Z",
    status: "COMPLETED",
    duration: 40,
    notes: "Focus technique et respiration.",
    createdAt,
    updatedAt,
    exercises: [
      {
        id: "bypass-workout-exercise-row",
        exerciseId: "bypass-exercise-row",
        order: 1,
        exercise: bypassExercises[2],
        sets: [
          {
            id: "bypass-set-row-1",
            setNumber: 1,
            reps: 12,
            weight: 18,
            rpe: 7,
            rir: 3,
            rest: 75,
            createdAt,
          },
          {
            id: "bypass-set-row-2",
            setNumber: 2,
            reps: 12,
            weight: 20,
            rpe: 8,
            rir: 2,
            rest: 90,
            createdAt,
          },
        ],
      },
      {
        id: "bypass-workout-exercise-plank",
        exerciseId: "bypass-exercise-plank",
        order: 2,
        exercise: bypassExercises[3],
        sets: [
          {
            id: "bypass-set-plank-1",
            setNumber: 1,
            reps: 45,
            weight: 0,
            rpe: 8,
            rir: 2,
            rest: 60,
            createdAt,
          },
          {
            id: "bypass-set-plank-2",
            setNumber: 2,
            reps: 40,
            weight: 0,
            rpe: 8.5,
            rir: 1,
            rest: 60,
            createdAt,
          },
        ],
      },
    ],
  },
];

export const bypassWorkoutTemplates: WorkoutTemplate[] = [
  {
    id: "bypass-template-cardio-debutant",
    name: "Cardio debutant",
    category: "Cardio",
    level: "Debutant",
    duration: 20,
    description: "Seance cardio simple pour reprendre progressivement.",
    displayOrder: 1,
    createdAt,
    updatedAt,
    exercises: [
      {
        id: "bypass-template-cardio-row",
        exerciseId: bypassExercises[3].id,
        order: 0,
        sets: 3,
        reps: 0,
        durationSeconds: 300,
        rest: 60,
        weight: 0,
        exercise: {
          id: bypassExercises[3].id,
          name: "Planche",
          description: bypassExercises[3].description,
          difficulty: "BEGINNER",
          exerciseType: "CARDIO",
          createdAt,
          updatedAt,
        },
      },
    ],
  },
  {
    id: "bypass-template-full-body",
    name: "Full body poids du corps debutant",
    category: "Poids du corps",
    level: "Debutant",
    duration: 25,
    description: "Base complete sans charge pour construire une routine.",
    displayOrder: 2,
    createdAt,
    updatedAt,
    exercises: [
      {
        id: "bypass-template-full-body-squat",
        exerciseId: bypassExercises[0].id,
        order: 0,
        sets: 3,
        reps: 10,
        durationSeconds: null,
        rest: 60,
        weight: 0,
        exercise: {
          id: bypassExercises[0].id,
          name: bypassExercises[0].name,
          description: bypassExercises[0].description,
          difficulty: "BEGINNER",
          exerciseType: "STRENGTH",
          createdAt,
          updatedAt,
        },
      },
      {
        id: "bypass-template-full-body-plank",
        exerciseId: bypassExercises[3].id,
        order: 1,
        sets: 3,
        reps: 0,
        durationSeconds: 30,
        rest: 60,
        weight: 0,
        exercise: {
          id: bypassExercises[3].id,
          name: bypassExercises[3].name,
          description: bypassExercises[3].description,
          difficulty: "BEGINNER",
          exerciseType: "STRENGTH",
          createdAt,
          updatedAt,
        },
      },
    ],
  },
  {
    id: "bypass-template-push",
    name: "Push",
    category: "Musculation",
    level: "Intermediaire",
    duration: 60,
    description: "Pectoraux, epaules et triceps.",
    displayOrder: 3,
    createdAt,
    updatedAt,
    exercises: [
      {
        id: "bypass-template-push-bench",
        exerciseId: bypassExercises[1].id,
        order: 0,
        sets: 4,
        reps: 8,
        durationSeconds: null,
        rest: 120,
        weight: 0,
        exercise: {
          id: bypassExercises[1].id,
          name: bypassExercises[1].name,
          description: bypassExercises[1].description,
          difficulty: "INTERMEDIATE",
          exerciseType: "STRENGTH",
          createdAt,
          updatedAt,
        },
      },
    ],
  },
  {
    id: "bypass-template-pull",
    name: "Pull",
    category: "Musculation",
    level: "Intermediaire",
    duration: 60,
    description: "Dos et biceps.",
    displayOrder: 4,
    createdAt,
    updatedAt,
    exercises: [
      {
        id: "bypass-template-pull-row",
        exerciseId: bypassExercises[2].id,
        order: 0,
        sets: 4,
        reps: 8,
        durationSeconds: null,
        rest: 120,
        weight: 0,
        exercise: {
          id: bypassExercises[2].id,
          name: bypassExercises[2].name,
          description: bypassExercises[2].description,
          difficulty: "INTERMEDIATE",
          exerciseType: "STRENGTH",
          createdAt,
          updatedAt,
        },
      },
    ],
  },
  {
    id: "bypass-template-legs",
    name: "Legs",
    category: "Musculation",
    level: "Intermediaire",
    duration: 60,
    description: "Quadriceps, ischios et mollets.",
    displayOrder: 5,
    createdAt,
    updatedAt,
    exercises: [
      {
        id: "bypass-template-legs-squat",
        exerciseId: bypassExercises[0].id,
        order: 0,
        sets: 4,
        reps: 8,
        durationSeconds: null,
        rest: 150,
        weight: 0,
        exercise: {
          id: bypassExercises[0].id,
          name: bypassExercises[0].name,
          description: bypassExercises[0].description,
          difficulty: "INTERMEDIATE",
          exerciseType: "STRENGTH",
          createdAt,
          updatedAt,
        },
      },
    ],
  },
];

export const bypassFoods: Food[] = [
  {
    id: "11111111-1111-4111-8111-111111111111",
    userId: bypassUserId,
    name: "Riz basmati",
    brand: null,
    barcode: null,
    caloriesKcal: 350,
    proteinGrams: 7,
    carbsGrams: 78,
    fatGrams: 1,
    fiberGrams: 2,
    servingUnit: "g",
    isGlobal: false,
    createdAt,
    updatedAt,
  },
  {
    id: "22222222-2222-4222-8222-222222222222",
    userId: bypassUserId,
    name: "Poulet grille",
    brand: null,
    barcode: null,
    caloriesKcal: 165,
    proteinGrams: 31,
    carbsGrams: 0,
    fatGrams: 4,
    fiberGrams: null,
    servingUnit: "g",
    isGlobal: false,
    createdAt,
    updatedAt,
  },
  {
    id: "33333333-3333-4333-8333-333333333333",
    userId: null,
    name: "Banane",
    brand: null,
    barcode: null,
    caloriesKcal: 89,
    proteinGrams: 1,
    carbsGrams: 23,
    fatGrams: 0,
    fiberGrams: 3,
    servingUnit: "g",
    isGlobal: true,
    createdAt,
    updatedAt,
  },
];

export const bypassMeals: Meal[] = [
  {
    id: "44444444-4444-4444-8444-444444444444",
    userId: bypassUserId,
    name: "Dejeuner entrainement",
    date: "2026-05-04T12:15:00.000Z",
    mealType: "lunch",
    notes: "Repas simple avant la seance.",
    createdAt,
    updatedAt,
    items: [
      {
        id: "55555555-5555-4555-8555-555555555555",
        foodId: bypassFoods[0].id,
        foodName: bypassFoods[0].name,
        quantityGrams: 150,
        caloriesKcalPer100g: bypassFoods[0].caloriesKcal,
        proteinGramsPer100g: bypassFoods[0].proteinGrams,
        carbsGramsPer100g: bypassFoods[0].carbsGrams,
        fatGramsPer100g: bypassFoods[0].fatGrams,
        totals: {
          caloriesKcal: 525,
          proteinGrams: 10.5,
          carbsGrams: 117,
          fatGrams: 1.5,
        },
        createdAt,
      },
      {
        id: "66666666-6666-4666-8666-666666666666",
        foodId: bypassFoods[1].id,
        foodName: bypassFoods[1].name,
        quantityGrams: 120,
        caloriesKcalPer100g: bypassFoods[1].caloriesKcal,
        proteinGramsPer100g: bypassFoods[1].proteinGrams,
        carbsGramsPer100g: bypassFoods[1].carbsGrams,
        fatGramsPer100g: bypassFoods[1].fatGrams,
        totals: {
          caloriesKcal: 198,
          proteinGrams: 37.2,
          carbsGrams: 0,
          fatGrams: 4.8,
        },
        createdAt,
      },
    ],
    totals: {
      caloriesKcal: 723,
      proteinGrams: 47.7,
      carbsGrams: 117,
      fatGrams: 6.3,
    },
  },
];

export const bypassNutritionGoals: NutritionGoal[] = [
  {
    id: "77777777-7777-4777-8777-777777777777",
    userId: bypassUserId,
    name: "Maintien",
    startDate: "2026-05-01T00:00:00.000Z",
    endDate: null,
    dailyCaloriesKcal: 2400,
    dailyProteinGrams: 160,
    dailyCarbsGrams: 260,
    dailyFatGrams: 70,
    isActive: true,
    createdAt,
    updatedAt,
  },
];
